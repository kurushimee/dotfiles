...
Coordinate sequential forks to document a recently solved problem.

## Purpose

Captures problem solutions while context is fresh, creating structured documentation in `docs/solutions/` with YAML frontmatter for searchability and future reference. Uses sequential forks for maximum efficiency.
...
When dispatching forks, pass the relevant file contents into the task prompt so they have the contract without needing cross-skill paths.
...
Before launching Phase 1 forks, check the auto-memory block injected into your system prompt for notes relevant to the problem being documented.
...
Launch these research forks at `effort: fast`. Each returns text data to the orchestrator. Independent research forks may run in parallel; run a fork after another only when the earlier fork's findings would sharpen its prompt or prevent duplicate work.

**Dispatch order:**
1. `Context Analyzer`
2. `Solution Extractor`
3. `Related Docs Finder`
4. Session-history discovery/extraction/synthesis flow — only if the user opted in to session history

Carry each completed fork's relevant findings into later fork prompts when they would sharpen scope or prevent duplicate work.
...
#### 3. **Related Docs Finder**
   ...
   **Search strategy (grep-first filtering for efficiency):**
   ...
   3. Use the native content-search tool (e.g., Grep in Claude Code) to pre-filter candidate files BEFORE reading any content. Run targeted searches in parallel, case-insensitive, targeting frontmatter fields. These are template patterns -- substitute actual keywords:
      - `title:.*<keyword>`
...
#### 4. **Session History** (internal flow — only if the user opted in)
   ...
   - Read the skill-local synthesis prompt at `references/agents/session-historian.md`, then dispatch a generic fork at `effort: fast` using that prompt content.
   ...
   **Extraction pipeline.** Create `SCRATCH=$(mktemp -d -t ce-compound-sessions-XXXXXX)`. For each selected session, write extracted content to scratch files:
   ...
   Use `extract-errors.py` selectively when dead ends or recurring errors are likely useful. Pass only the scratch file paths and metadata to the synthesis fork.

   **Synthesis dispatch.** Build a generic fork prompt containing:
   ...
   The `effort: fast` fork reads only the scratch paths and returns prose findings. If synthesis fails, note the failure and continue without session context.

### Phase 2: Assembly & Write

<sequential_tasks>

**WAIT for all Phase 1 inputs to complete before proceeding** — the Phase 1 research forks and, when the user opted in, the internal session-history flow. Session history is a Phase 1 input even though it runs in the orchestrator rather than as a public skill.

The orchestrating agent (main conversation) performs these steps:

1. Collect all text results from Phase 1 forks
...
Based on problem type, optionally dispatch generic forks at `effort: balanced`, seeded with local prompt assets from `references/agents/`, to review the documentation. Independent reviewers may run in parallel.
...
- Any code-heavy issue → preserve code simplification as a **read-only documentation review**. Inspect the solution draft's code examples and explanatory claims inline, or dispatch an `effort: balanced` fork seeded with a local prompt only to return suggestions. Do **not** invoke `ce-simplify-code` from this phase and do not mutate product code unless the user explicitly asks for a separate code-simplification pass.
...
This mode skips sequential forks entirely. The orchestrator performs all work in a single pass, producing the same solution document without cross-referencing or duplicate detection.
...
**The solution doc is the one deliverable** (Phase 2.4's update-only vocabulary capture may also refine an existing `CONCEPTS.md`).

In lightweight mode, the overlap check is skipped (no Related Docs Finder fork). This means lightweight mode may create a doc that overlaps with an existing one. That is acceptable — `ce-compound-refresh` will catch it later. Only suggest `ce-compound-refresh` if there is an obvious narrow refresh target. Do not broaden into a large refresh sweep from a lightweight session.
...
| ❌ Wrong | ✅ Correct |
|----------|-----------|
| Forks write files like `context-analysis.md`, `solution-draft.md` | Forks return text data; orchestrator writes one final file |
| Research and assembly run sequentially | Research completes → then assembly runs |
...
### When to Invoke
- **Auto-triggered** (optional): `effort: balanced` forks seeded with local prompts can run post-documentation for enhancement
