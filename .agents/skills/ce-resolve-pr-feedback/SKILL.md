...
description: Resolve PR review feedback by evaluating validity and fixing issues with forks. Use when addressing PR review comments, resolving review threads, or fixing code review feedback.
...
Evaluate and fix PR review feedback, then reply and resolve threads. Dispatch `effort: balanced` generic forks, seeded with a skill-local resolver prompt for each thread.
...
- **Full Mode** → `references/full-mode.md` (9 steps: fetch, triage, plan, implement, validate, commit/push, reply/resolve, verify, summary)
...
- Resolver prompt asset → `references/agents/pr-comment-resolver.md` (read before dispatching `effort: balanced` resolver forks; use it as context for a generic fork)
