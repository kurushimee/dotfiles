...
**For review threads** (`review_threads`): Read `references/agents/pr-comment-resolver.md` and dispatch one `effort: balanced` generic fork seeded with that prompt for each new thread. Resolver forks that touch disjoint files may run in parallel; items that touch the same file run one at a time so each sees the previous diff.

Each resolver fork receives:
- The thread ID
...
Process items in priority order: review threads first, then PR comments, then review bodies. Within each group, handle higher-risk or higher-severity items first when that can be inferred from the comment.

Before each resolver dispatch, pass the current diff state and any previously resolved item summaries that may affect the same files. When multiple items reference the same file, either assign them to one resolver fork for that file or run separate resolver forks back-to-back so each sees the previous diff.

After each resolver returns, review the changed files and keep the working tree coherent before dispatching the next resolver.

## 5. Validate Combined State

After all resolver forks complete, aggregate `files_changed` across every returned summary. If it's empty -- all verdicts are `replied`, `not-addressing`, `declined`, or `needs-human` -- skip steps 5 and 6 entirely and proceed to step 7.

Resolvers run only targeted tests on their own changes. This step runs the project's full validation **once** against the combined diff to catch interactions between fixes.
...
1. Stage only files reported by forks and commit with a message referencing the PR:
