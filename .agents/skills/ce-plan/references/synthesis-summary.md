...
- **Solo variant** (Phase 0.7): fires after Phase 0.4 bootstrap and Phase 0.6 depth classification, before Phase 1 research begins. Catches scope misinterpretation before fork dispatch is spent. Full breadth — problem frame, intended behavior, success criteria, in/out scope.
...
**Why pre-research, not pre-write**: research effort would be wasted if scope is wrong. Catching scope errors before fork dispatch (Phase 1.1's repo-research-analyst, learnings-researcher, etc.) saves token and time cost.
