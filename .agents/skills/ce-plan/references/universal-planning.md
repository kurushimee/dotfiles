...
Carry out the approach. When the answer depends on facts the model can't reliably supply from memory — current data, recent events, specifics that drift — gather them using the **Research decomposition pattern** under Step 1 below (decompose into focused questions, answer them with `effort: fast` fork/web research — independent questions run in parallel, sequential when one answer feeds the next — then collate). Skip research for anything the model already knows well.
...
2. Dispatch research at `effort: fast`; independent questions run in parallel, sequential when one answer feeds the next. Prefer user-named surfaces first per Core Principle 8 in SKILL.md; fall back to web search for questions those surfaces don't cover.
