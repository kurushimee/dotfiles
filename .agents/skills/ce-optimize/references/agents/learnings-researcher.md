...
**Use the native content-search tool (e.g., Grep in Claude Code) to find candidate files BEFORE reading any content.** Run focused searches in parallel, case-insensitive, returning only matching file paths:
...
content-search: pattern="tags:.*(fork|orchestration|token-efficiency)" path=docs/solutions/ files_only=true case_insensitive=true
...
- Use `|` for synonyms: `tags:.*(fork|orchestration|delegation)` or `tags:.*(payment|billing|stripe|subscription)`
...
- Run focused content searches in parallel across different keyword dimensions
...
- Skip focused parallel search before reading candidate files
