...
3. **Code Review**
   ...
   **2b. Apply fixes (caller-owned).** Load `references/review-findings-followup.md`: filter on JSON, batch by file, dispatch fix forks. Orchestrator merges, tests, commits. Then proceed to the Residual Work Gate.
...
4. **Residual Work Gate** (REQUIRED when Tier 2 ran)
   ...
   - `Apply/fix now` — load `references/review-findings-followup.md`, dispatch batched fix forks for remaining eligible findings, run tests, commit if needed; optionally re-run `ce-code-review` only after the diff changed materially.
...
**Tier 2 -- `ce-code-review` (escalation).** (2a) Review-only via `mode:agent`. (2b) Batched fix forks per `references/review-findings-followup.md`; residuals → Residual Work Gate.
