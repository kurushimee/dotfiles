...
- **Hardcoded operational settings** (15-minute trailing buffer, top-N error count, p50/p95/p99 latencies, "no PII in reports", serial analytics/tracing/payments/DB queries): these are skill behavior, not user config; they live in `SKILL.md` and stay constant.
