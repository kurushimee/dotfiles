---
name: ce-doc-review
description: Review requirements or plan documents using persona forks that surface role-specific issues. Use when a requirements document or plan document exists and the user wants to improve it.
argument-hint: "[mode:headless] [path/to/document.md]"
---

# Document Review

Review requirements or plan documents through multi-persona analysis. Dispatches generic forks seeded with skill-local reviewer prompt assets, auto-applies `safe_auto` fixes, and routes remaining findings through a four-option interaction (per-finding walk-through, auto-resolve with best judgment, Append-to-Open-Questions, Report-only) for user decision.
...
**If no document is specified (headless mode):** Output "Review failed: headless mode requires a document path. Re-invoke with: Skill(\"ce-doc-review\", \"mode:headless <path>\")" without dispatching forks.
...
Pass the classification result to each persona via the `{document_type}` slot in the fork template. Personas read this and adapt their analysis accordingly.
...
Dispatch generic forks with the Fork tool. Omit the `mode` parameter so the user's configured permission settings apply. Dispatch the `effort: balanced` reviewers in parallel and collect their returns as they complete; dispatch the `effort: deep` reviewers one at a time: dispatch one, wait for its return, record success or failure, then dispatch the next.

For each selected reviewer, read the matching skill-local prompt asset at `references/personas/<reviewer-name>.md` and pass its full content as `{persona_file}`. Use the prompt asset as context for a generic fork rather than relying on platform-level reviewer registration.
...
- `coherence-reviewer`: cheapest capable extraction/reasoning tier; dispatch at `effort: balanced`.
- `design-lens-reviewer`, `security-lens-reviewer`, `scope-guardian-reviewer`: platform mid-tier model; dispatch at `effort: balanced`. These and `coherence-reviewer` may run in parallel.
- `feasibility-reviewer`, `product-lens-reviewer`, `adversarial-document-reviewer`: inherit the parent model unless the harness has an established high-capability review tier; dispatch at `effort: deep`, one at a time.

Each fork receives the prompt built from the fork template included below with these variables filled:
...
Pass each fork the **full document** — do not split into sections.
...
**Error handling:** If a fork fails or times out, proceed with findings from forks that completed. Note the failed reviewer in the Coverage section. Do not block the entire review on a single reviewer failure.

**Dispatch order:** Dispatch the `effort: balanced` reviewers in parallel; dispatch each `effort: deep` reviewer one at a time, completing one before launching the next.

## Phases 3-5: Synthesis, Presentation, and Next Action

After all dispatched forks return, read `references/synthesis-and-presentation.md` for the synthesis pipeline (validate, anchor-based gate, dedup, cross-persona agreement promotion, resolve contradictions, auto-promotion, route by three tiers with FYI subsection), `safe_auto` fix application, headless-envelope output, and the handoff to the routing question.

For the four-option routing question and per-finding walk-through (interactive mode), read `references/walkthrough.md`. For the bulk-action preview used by best-judgment routing, Append-to-Open-Questions, and walk-through `Auto-resolve with best judgment on the rest`, read `references/bulk-preview.md`. Do not load these files before fork dispatch completes.
