...
- **Replace territory** — the recommended solution conflicts with current code, the architectural approach changed, or the pattern is no longer the preferred way. This means a new learning needs to be written. An `effort: balanced` replacement fork writes the successor following `ce-compound`'s document format (frontmatter, problem, root cause, solution, prevention), using the investigation evidence already gathered. The orchestrator does not rewrite learnings inline — it delegates to a fork for context isolation.
...
Use forks for context isolation when investigating multiple artifacts — not just because the task sounds complex. Choose the lightest approach that fits:

| Approach | When to use |
|----------|-------------|
| **Main thread only** | Small scope, short docs |
| **`effort: fast` forks** | 1-2 artifacts with many supporting files to read |
| **`effort: fast` forks** | 3+ artifacts — investigate one artifact or tight artifact group at a time; independent artifacts run in parallel |
| **Scoped passes** | Broad sweeps — narrow scope first, then run investigation passes |

**When dispatching any fork**, omit the `mode` parameter so the user's configured permission settings apply. Include this instruction in its task prompt:
...
There are two fork roles:

1. **Investigation forks** — read-only, `effort: fast`. They must not edit files, create successors, or delete anything. Each returns: file path, evidence, recommended action, confidence, and open questions. Independent investigations on disjoint artifacts run in parallel; investigate overlapping artifacts together or in sequence.
2. **Replacement forks** — write a single new learning to replace a stale one, `effort: balanced`. Run one replacement fork at a time. The orchestrator handles all deletions and metadata updates after each replacement completes.

The orchestrator merges investigation results, detects contradictions, coordinates replacement forks, and performs all deletions/metadata edits centrally. In interactive mode, it asks the user questions on ambiguous cases. In headless mode, it marks ambiguous cases as stale instead. If two artifacts overlap or discuss the same root issue, investigate them together.
...
- **Replace** — write a successor learning via fork (passing the documentation contract files), validate frontmatter, then delete the old. When evidence is insufficient, mark stale instead.
