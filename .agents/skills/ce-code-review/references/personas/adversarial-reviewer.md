...
- **Concurrent mutation** -- two users edit the same resource one at a time, two processes claim the same job, two requests update the same counter.
...
Use the anchored confidence rubric in the fork template. Persona-specific guidance:
...
**Anchor 25 or below — suppress** — the scenario requires conditions you have no evidence for: pure speculation about runtime state, theoretical cascades without traceable steps, or failure modes that require multiple unlikely conditions one at a time.
