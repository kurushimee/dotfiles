...
14 reviewer personas organized into always-on, cross-cutting conditional, and stack-specific conditional layers, plus CE-specific local prompt assets. The orchestrator uses this catalog to select which reviewers to dispatch for each review.

## Always-on (4 structured personas + 2 local prompt assets)

Dispatched on every review regardless of diff content.
...
Dispatched when the orchestrator identifies relevant patterns in the diff. The orchestrator reads the full diff and reasons about selection -- this is judgment, not keyword matching.
...
These reviewers cover runtime behavior the always-on personas do not specialize in. Structural and maintainability concerns live in the always-on `maintainability` persona.
...
1. **Always dispatch all 4 always-on personas** plus the 2 CE always-on local prompt assets.
...
4. **For `data-migration`**, select only when the diff includes migration or schema artifacts (`db/migrate/*`, `db/schema.rb`, `db/structure.sql`, Alembic/Flyway/Liquibase paths, or explicit backfill/data-transform scripts). Skip model-only or query-only changes without those files.
...
6. **Announce the team** before dispatch with a one-line justification per conditional reviewer selected.
