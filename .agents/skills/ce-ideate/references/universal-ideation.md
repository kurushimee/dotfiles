...
- **Full** — rich intake, multiple frames run across tiers (cheap frames in parallel, ceiling frames one at a time), deep critique, present 5-7 survivors with strong rationale.
...
**Grounding freshness.** Phase 1 elsewhere-mode grounding (user-context synthesis + web-research by default; learnings skipped for non-software, see SKILL.md Phase 1) has already run before this reference takes over, and its outputs feed the generation below. If intake answers here materially refine the topic or constraints — new scope, different audience, a domain shift that the original grounding did not cover — re-dispatch the affected Phase 1 forks on the refined topic before generating ideas. The guardrail mirrors SKILL.md Phase 0.4's rule that mode and grounding re-evaluate when intake changes the scope to be acted on; ranking against stale grounding risks surfacing ideas fit to the wrong topic.
...
This step is the facilitator's own analysis — no fork, no additional research. The Phase 1 grounding supplies the substance.
...
Generate the full candidate list before critiquing any idea. Use the same six frames as software ideation, described in domain-agnostic language. Each frame is a **starting bias, not a constraint** — follow promising threads across frames. When dispatching frames as forks (Full depth), follow SKILL.md Model Tiers: evidence-driven frames (pain, inversion, leverage) on the generation tier at `effort: balanced` (independent frames run in parallel); ceiling frames (assumption-breaking, analogy, constraint-flipping) on the ceiling tier at `effort: deep`, one at a time.
