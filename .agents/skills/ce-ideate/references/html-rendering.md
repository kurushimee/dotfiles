...
1. Repository root (resolve via `git rev-parse --show-toplevel`).
2. `docs/DESIGN.md`.
3. `.compound-engineering/DESIGN.md`.

Read once at compose time. Absent → fall through to the fallback default.

Repository-root only — do not fall through to a main checkout. Users
working from a checkout who want HTML defaults can add DESIGN.md to the
checkout.
